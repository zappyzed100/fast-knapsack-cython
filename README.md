...
- 10秒以降で Cython の進化計算が大きく伸び、30秒〜60秒では Cbc に近い平均スコアを示す
- Cython が強い場面が多い主因は、Cython は純粋な計算速度が非常に高速であるため、同時間内で反復回数・世代交代回数をより確保しやすい点にある
- 進化計算は単体焼きなまし法（SA）より解のばらつきが大きく、多様な候補解を探索できる点が、同一条件で同種の解に収束しやすい決定論的ソルバーに対する実務上の強みになっている

本リポジトリの主張は「常に厳密解法を上回る」ことではありません。短時間で高品質な実行可能解を返せるヒューリスティック実装を、汎用ソルバーと比較可能な形で提示すること、加えて Cython / Numba の実装自由度（制約追加や探索戦略調整のしやすさ）と、進化計算が持つ解の多様性を示すことにあります。
...
図の横軸（0.03, 1, 10, 30, 60秒）は、実時間比ではなく指定順で等間隔に配置しています。CBC差分スコアは `各手法平均 - CBC平均` です。
